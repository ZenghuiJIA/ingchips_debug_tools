        ...
        for line in lines:
            ...
            if in_mem_map:
                ...
                if sline.startswith("Execution Region"):
                    m = re.search(r"Execution Region\s+(\w+)\s+\((?:Exec\s+)?base:\s*(0x[0-9a-fA-F]+).*?Size:\s*(0x[0-9a-fA-F]+)", sline, re.IGNORECASE)
                    if m:
                ...
                if cur_region and sline.startswith("0x"):
                    ...
                    if len(parts) >= 3 and parts[2].startswith("0x"):
                        # Format 2 (Exec Addr, Load Addr, Size, Type, Attr, ...)
                        sz = int(parts[2], 16)
                        if len(parts) >= 4 and parts[3] == "PAD":
                            padding_bytes += sz
                            ...
                            t = parts[3]
                            attr = parts[4] if len(parts) > 4 else "RO"
                            if len(parts) >= 8 and parts[6] == "*":
                                sec_name = parts[7]
                            elif len(parts) >= 7:
                                sec_name = parts[6]
                            else:
                                sec_name = parts[5] if len(parts) >= 6 else ""
                            obj_name = parts[-1]
                            regions[cur_region]["entries"].append({
                                "addr": parts[0],
                                "addr_int": int(parts[0], 16),
                        ...
                        # Format 1 (Base Addr, Size, Type, Attr, ...)
                        sz = int(parts[1], 16)
                        if len(parts) >= 3 and parts[2] == "PAD":
                            ...
                            t = parts[2]
                            attr = parts[3] if len(parts) > 3 else "RO"
                            if len(parts) >= 7 and parts[5] == "*":
                                sec_name = parts[6]
                            elif len(parts) >= 6:
                                sec_name = parts[5]
                            else:
                                sec_name = parts[4] if len(parts) >= 5 else ""
                            obj_name = parts[-1]
                            regions[cur_region]["entries"].append({
                                "addr": parts[0],
                                "addr_int": int(parts[0], 16),
            ...
            if "Grand Totals" in sline:
                # Format: Code (inc. data)   RO Data    RW Data    ZI Data      Debug   Grand Totals
                # e.g.: 184408      19626      14916       1012      20840    1022301   Grand Totals
                parts = sline.split()
                if len(parts) >= 6 and parts[-1] == "Totals":
                    try:
                        code_bytes = int(parts[0])
                        ro_data_bytes = int(parts[2])
                        rw_data_bytes = int(parts[3])
                        zi_data_bytes = int(parts[4])
