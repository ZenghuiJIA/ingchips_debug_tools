...
### 1. 启动调试工具
无需任何启动脚本，**直接双击运行 `AI-HIL-Debugger.exe`** 即可：
- 🌟 **启动自动净化**：打开时内置进程看门狗会自动检测并清理之前可能残留的旧后台守护进程，保证端口与探针独占权立即可用。
- 🌟 **退出自动级联回收**：直接点击右上角叉掉（关闭）窗口时，自动无死角终止并回收底层 Python / PyOCD 守护进程及串口连接，不再遗留任何僵尸进程。
- *(注：原 `start.bat` 依然保留作为可选诊断启动器)*
